            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        <h3 class="pull-left"><br>Edycja użytkowników</h3>
                        <br><a href="#one" class="btn btn-success pull-right">Dodaj lub usuń użytkownika</a>
						
             

			
<form action="uzytkins.php" method="post">
<table class='table table-bordered table-striped'><tr><td></td><td><b>NR</b></td><td><b>Login</b></td><td><b>Hasło</b></td><td><b>Email</b></td></tr>
<?php
$numerWiersza=1;
$z =  mysqli_query($db,"SELECT * FROM `uzytkownicy`");
while($tab=mysqli_fetch_row($z))
{echo '<tr><td><button type="submit"  name="edytowanie" value='.$tab[0].' onclick="edytujUzytk('.$numerWiersza.')" class="edycja">E</button>
...
<td><input type=text class="pole" name=login></td>
<td><input type=text class="pole" name=haslo></td>
<td><input type=text class="pole" name=email></td></tr>
</table>
<br>
</form>
<script>
function edytujUzytk(wiersz) {
